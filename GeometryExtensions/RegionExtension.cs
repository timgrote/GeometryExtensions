...
namespace AID.GeometryExtensions
{
	...
	public static class RegionExtension
	{
		...
		{
			using (var brep = new BoundRep.Brep(region))
			{
			...
			using (var brep = new BoundRep.Brep(region))
			{
				foreach (var complex in brep.Complexes)
				{
					foreach (var loop in complex.Shells.First().Faces.First().Loops)
					{
						...
						if (loop.LoopType == BoundRep.LoopType.LoopExterior)
		...
		public static BoundRep.PointContainment GetPointContainment(this Region region, Point3d point)
		{
			using (BoundRep.Brep brep = new BoundRep.Brep(region))
			using (BoundRep.BrepEntity entity = brep.GetPointContainment(point, out BoundRep.PointContainment result))
			{
				switch (entity)
				{
					case BoundRep.Face _:
						return BoundRep.PointContainment.Inside;
					case BoundRep.Edge _:
						return BoundRep.PointContainment.OnBoundary;
					default:
						return BoundRep.PointContainment.Outside;
				}
			}
		}
	}
